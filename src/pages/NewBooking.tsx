            ...
            <div className="flex gap-4 pt-1">
              {["AM", "PM"].map((s) => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="slot" checked={form.allottedSlot === s} onChange={() => set("allottedSlot", s)} className="accent-primary" />
            ...
            <div className="flex gap-4 pt-1">
              {["Single", "Double"].map((h) => (
                <label key={h} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="hall" checked={form.hallType === h} onChange={() => set("hallType", h)} className="accent-primary" />
        ...
        <CardContent className="space-y-4">
          <div className="bg-muted rounded-lg p-4 max-h-48 overflow-y-auto text-xs space-y-1.5 text-muted-foreground">
            {[
              ...
              "Extra hours will be charged as per prevailing rates.",
              "The advance amount is non-refundable in case of cancellation.",
              "The management reserves the right to cancel with prior notice.",
              "Electricity charges for extra equipment will be borne by the applicant.",
              "The applicant must follow all government guidelines.",
              "Children must be supervised by parents/guardians at all times.",
              "The management is not liable for any accidents.",
